...
void MediaStreamDiscordVideoSource::VideoSourceDelegate::
  ...
  const gpu::SharedImageInfo si_info(
      viz::SinglePlaneFormat::kRGBA_8888, gfx::Size(),
      gfx::ColorSpace::CreateSRGB(), kTopLeft_GrSurfaceOrigin,
      kPremul_SkAlphaType, usage, "DiscordVideoSource");
  scoped_refptr<gpu::ClientSharedImage> client_shared_image =
      sii->CreateSharedImage(si_info, buffer_impl->CloneHandle());
  scoped_refptr<media::VideoFrame> video_frame =
      media::VideoFrame::WrapExternalGpuMemoryBuffer(
          gfx::Rect(size), size, std::move(buffer_impl), client_shared_image,
          gpu::SyncToken(), client_shared_image->GetTextureTarget(),
